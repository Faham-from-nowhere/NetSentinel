...
# This is the lightweight alert for the WebSocket
class Alert(BaseModel):
    incident_id: str
    threat_score: int
    main_event: str
    status: str
    sequence: List[IncidentSequenceItem] # Will just contain the first event
    ai_summary: str

# This is the full incident model for the API 
class FullIncident(BaseModel):
    ...
    last_seen: float
    attacker_ip: str
    sequence: List[IncidentSequenceItem] # The full story
    ai_summary: Optional[str] = None # The summary might not be generated yet
    ...
    try:
        while True:
            if anomaly_alerts_queue:
                ...
                # Add summary to both the main DB and the outgoing alert
                ...
                # Validate and send the *lightweight* alert
...
def get_incident_details(incident_id: str):
    ...
    # Validate with our Pydantic model before sending
    return FullIncident(**incident)


# 6. Root Endpoint (UNCHANGED)
@app.get("/")
def read_root():
